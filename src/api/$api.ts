...
import type { Methods as Methods1 } from './v1/blogs/detail/_id'
import type { Methods as Methods2 } from './v1/musics'
import type { Methods as Methods3 } from './v1/musics/detail/_id@string'
import type { Methods as Methods4 } from './v1/musics/song-categories'
import type { Methods as Methods5 } from './v1/musics/utils/cms-webhook'
import type { Methods as Methods6 } from './v1/musics/utils/iframe-converter'
          ...
          _id: (val3: number | string) => {
            ...
            return {
              ...
              options: (option?: { config?: T | undefined } | undefined) =>
                fetch<void, Methods1['options']['resHeaders'], Methods1['options']['status']>(prefix, prefix3, OPTIONS, option).send(),
              /**
               * APIGateway上のCORS用設定
               */
              $options: (option?: { config?: T | undefined } | undefined) =>
                fetch<void, Methods1['options']['resHeaders'], Methods1['options']['status']>(prefix, prefix3, OPTIONS, option).send().then(r => r.body),
              $path: () => `${prefix}${prefix3}`
            }
          }
        },
        ...
        detail: {
          _id: (val3: string) => {
            ...
            return {
               ...
               */
              get: (option?: { config?: T | undefined } | undefined) =>
                fetch<Methods3['get']['resBody'], Methods3['get']['resHeaders'], Methods3['get']['status']>(prefix, prefix3, GET, option).json(),
              /**
               * MicroCMSに登録されている曲の取得
               * @returns 成功時のレスポンス
               */
              $get: (option?: { config?: T | undefined } | undefined) =>
                fetch<Methods3['get']['resBody'], Methods3['get']['resHeaders'], Methods3['get']['status']>(prefix, prefix3, GET, option).json().then(r => r.body),
              /**
               * APIGateway上のCORS用設定
               */
              options: (option?: { config?: T | undefined } | undefined) =>
                fetch<void, Methods3['options']['resHeaders'], Methods3['options']['status']>(prefix, prefix3, OPTIONS, option).send(),
              /**
               * APIGateway上のCORS用設定
               */
              $options: (option?: { config?: T | undefined } | undefined) =>
                fetch<void, Methods3['options']['resHeaders'], Methods3['options']['status']>(prefix, prefix3, OPTIONS, option).send().then(r => r.body),
              $path: () => `${prefix}${prefix3}`
            }
          }
        },
